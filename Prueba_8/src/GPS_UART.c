    ...
    while(true){
        ...
        */
        ...
        if(rxbytes>0){// Si rxbytes es distinto de cero ejecuta el siguiente bloque de codigo
            ...
            */
            p = strstr((const char*)data, "$GPGGA");// Localizo la posicion de la palabra definida en str en el puntero *data y la guardo en *p
            ...
            p = strstr((const char*)data, "$GPRMC");
            if(p){// Si *p es distinto de NULL ejecuto el codigo a continuacion
                //printf("Se alla RMC\n");
                for(int i=0; p[i] != '\n'; i++){// Recorro *p hasta detectar un salto de linea y guardo cada caracter en str2
                    buffertocopyinto[j] = p[i];// Guarda cada caracter en el array str2
            ...
            // else
            // {
            //     printf("No se alla RMC\n");
            // }
            
                /* Print messages */
                if(estadoModo == CONECTAR_GPS || estadoModo == GUARDAR_DATOS || estadoModo == CONECTAR_WIFI){
                    if (mux2 != NULL){
...
esp_err_t CrearTareaGPS_UART(void){
    ...
    BaseType_t res = xTaskCreatePinnedToCore(
        ...
        1024*3, 		    // Cantidad de stack de la tarea
