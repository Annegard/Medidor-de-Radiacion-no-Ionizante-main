...
void lcd_print_char(unsigned char c) {
    char buffer[2];  // Un carácter + terminador nulo
    buffer[0] = c;
    buffer[1] = '\0';  // Terminador de cadena
    lcd_print(buffer); // Usa `lcd_print` para enviar el carácter
}

void lcd_print_int(int number) {
    char buffer[16]; // Ajusta el tamaño según lo necesites
    sprintf(buffer, "%d", number);  // Convierte el entero a cadena
    lcd_print(buffer);  // Usa tu función `lcd_print`
}
