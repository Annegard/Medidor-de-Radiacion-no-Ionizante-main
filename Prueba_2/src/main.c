...
void app_main()
{
    ...
    // Los semáforos son útiles para detener una tarea, porque comparte un recurso con otras,
    // como el puerto serie. Los semáforos solo deben usarse mientras el planificador se está ejecutando,
    // pero podemos configurarlo aquí.
    // if (xSemaforo_Serial == NULL) // Confirma que el semáforo del puerto serial aún no se ha creado.
    // {
    //     xSemaforo_Serial = xSemaphoreCreateMutex(); // Se crea un semáforo mutex que se usa para administrar el puerto serial
    //     if ((xSemaforo_Serial) != NULL)
    //         xSemaphoreGive((xSemaforo_Serial)); // Hace que el puerto serie esté disponible para su uso, "Dando" el semáforo.
    // }
...
void TareaLCD(void *taskParmPtr)
{
    ...
    while (true)
    {
        ...
        {
            // Se pudo obtener o "Tomar" el semáforo y ahora se puede acceder al recurso compartido.
            // Se quiere tener el puerto serie solo para la tarea, ya que se necesita algo de tiempo para imprimir,
            // por lo que no se desea que otra tarea lo ocupa en la mitad de una transmisión.
            printf("Tarea LCD\r\n");

            xSemaphoreGive(xSemaforo_Serial); // Ahora cede el puerto serie a otras tareas.
        }
